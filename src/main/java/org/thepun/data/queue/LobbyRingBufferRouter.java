...
public final class LobbyRingBufferRouter<T> implements Router<T> {
    ...
    public LobbyRingBufferRouter(int bufferSize) {
        if (bufferSize < 1) {
    ...
    private static final class RingBufferProducer<T> implements QueueTail<T> {
        ...
        private final LobbyRingBufferRouter<T> parent;

        // gap
        private Object t1, t2;

        private RingBufferProducer(LobbyRingBufferRouter<T> parent) {
            this.parent = parent;
    ...
    private static final class RingBufferConsumer<T> implements QueueHead<T> {
        ...
        private RingBufferConsumer(LobbyRingBufferRouter<T> parent) {
            this.parent = parent;
